...
class RV32I (
 ...
 // Assign the data interface
  ...
  val addressWithLSB = io_reg.reg_read_data1 + decoder.io_decoder.imm

// Create a variable 'correct' which is all ones except the least significant bit set to 0
val correct = "hfffffffe".U(32.W)  // 32-bit value with all ones except LSB set to 0

// Perform a bitwise AND to set the LSB to 0
val addressWithLSBZero = addressWithLSB & correct

// Use the updated address for the PC write data
when(decoder.io_decoder.instr_type === RISCV_TYPE.jalr) {
  io_pc.pc_wdata := addressWithLSBZero
}
//wait for the memory to be finised with the data and then we load the data in the register
  /*when(io_data.data_gnt){
    io_reg.reg_write_en:=true.B
    io_reg.reg_write_data := io_data.data_rdata
  }*/
  when(decoder.io_decoder.instr_type === RISCV_TYPE.lw) {
  /*when((decoder.io_decoder.imm === 0.U) && (decoder.io_decoder.rd === decoder.io_decoder.rs1)) {
   //  Do nothing, retain the values of the destination register
     io_reg.reg_write_en := false.B // Optionally, explicitly disable writing
  } */
  io_reg.reg_write_en := false.B // Optionally, explicitly disable writing
    when(io_data.data_gnt) {
      io_reg.reg_write_en := true.B
